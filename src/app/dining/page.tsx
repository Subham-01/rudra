...
export default async function DiningPage() {
  ...
  const diningExperiences = getParsedContent('diningExperiences', defaultDiningExperiences);
  const featurePoints = getParsedContent('featurePoints', defaultFeaturePoints);
  const signatureMoments = getParsedContent('signatureMoments', defaultSignatureMoments);
  
  const menuPdfUrl = settings.dining_menu_pdf || "/pdf/2025 flavoresca-menu.pdf";
  ...
  const restaurantSchema = {
    ...
    telephone: settings.phone_number || "+91 8651600015",
    parentOrganization: {
      "@type": "Hotel",
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
        ...
        <div className="relative flex min-h-[62vh] items-center max-w-6xl mx-auto px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
          <div className="animate-fade-in-up max-w-3xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:gap-4">
              ...
              <Link href={createHotelInquiryLink("a dining reservation at The Flavoresca Restaurant", ["Location: Hotel Rudra Regency, Motihari", "Reservation type: Table booking"], whatsappNumber) } className="inline-flex items-center justify-center rounded-full border border-amber-300/30 bg-black/70 px-6 py-3 text-base font-semibold text-amber-200 transition hover:bg-black/90 sm:px-8">
                Reserve a Table
              </Link>
            </div>
          </div>
      ...
      <main className="relative mt-2 md:mt-4 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-3 sm:mt-12">
          {diningExperiences.map((item: any) => (
            <article
              key={item.title}
        ...
        <section className="mt-12 space-y-6 sm:mt-16 sm:space-y-8">
          {signatureMoments.map((moment: any, index: number) => (
            <div
              key={moment.title}
        ...
        <section className="mt-12 sm:mt-16">
          <div className="animate-fade-in-up flex min-h-[320px] items-center justify-center rounded-[28px] bg-black p-5 text-center text-white shadow-2xl sm:min-h-[360px] sm:rounded-[32px] sm:p-10">
            <div>
              ...
              <Link href={createHotelInquiryLink("a dining reservation at The Flavoresca Restaurant", ["Location: Hotel Rudra Regency, Motihari", "Reservation type: Table booking"], whatsappNumber) } className="inline-flex items-center gap-2 rounded-full bg-amber-400 px-6 py-3 text-sm font-semibold text-black shadow-lg hover:bg-amber-300 transition">
                Reserve your table
              </Link>
            </div>
          </div>
